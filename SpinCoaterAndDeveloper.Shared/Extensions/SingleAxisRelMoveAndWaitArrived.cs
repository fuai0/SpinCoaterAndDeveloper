...
{
    public class SingleAxisRelMoveAndWaitArrived
    {
        private readonly IMotionCardService motionCardService;
        private readonly ILogService logService;
        private CancellationTokenSource cancellationTokenSource;
        public SingleAxisRelMoveAndWaitArrived(IContainerProvider containerProvider)
        {
            this.motionCardService = containerProvider.Resolve<IMotionCardService>();
            this.logService = containerProvider.Resolve<ILogService>();
        }

        public async Task<bool> StartRelAsync(string axisName, double vel, double acc, double dec, double relVable, double timeout, CancellationTokenSource cancellationTokenSource)
        {
            this.cancellationTokenSource = cancellationTokenSource;
            var result = await Task.Run(() =>
            {
                motionCardService.StartMoveRel((short)MotionControlResource.AxisResource[axisName].AxisIdOnCard,
                                               0,
                                               0,
                                               ...
                                               relVable * MotionControlResource.AxisResource[axisName].Proportion);
